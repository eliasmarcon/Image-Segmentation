...
class IntersectOverUnion(Metric):
    ...
    def calculate_metric(self, local_rank: int) -> float:
        ...
        This version is adapted for multi-GPU training using PyTorch's Distributed Data Parallel.
        '''
        ...
        # Calculate IoU for each class
        iou = torch.zeros(self.num_classes, dtype=torch.float32, device=local_rank)
        ...
        torch.distributed.all_reduce(torch.tensor([mean_iou], device=local_rank), op=torch.distributed.ReduceOp.SUM)
